...
var XbmcModule = function(rpcHandler, params)
{
    ...
    this.base({
        ...
        description: 'Webinos Media-Play API, ' + params.device.friendlyName + ' ' + params.device.ip
    });
        ...
        switch(operation.toString())
        {
            ...
            case "volumeup":
                if(playerInstance.volume < volumeRange.max)
                {
                    ...
                    //callListeners("onVolumeUP", playerInstance.volume);
                }
                break;
            case "volumedown":
                if(playerInstance.volume > volumeRange.min)
                {
                    ...
                    //callListeners("onVolumeDOWN", playerInstance.volume);
                }
                break;
        }
...
XbmcModule.prototype.bindService = function()
{
    ...
    this.connection.callback = callListeners;
...
function callListeners(event, payload)
{
    var _event = {};
    ...
    if(event.result)
    {
        ...
        {
            fs.writeFile(wPath+"/userData/"+ this.displayName + "_Movies.json", JSON.stringify(event.result.movies));
            console.log(wPath+"/userData/"+ this.displayName + "_Movies.json");
    ...
    console.log("\n");
    console.log(_event.type, _event.payload);
