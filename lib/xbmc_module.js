...
XbmcModule.prototype.addListener = function(callbacks, successCB, errorCB, objectRef)
{
    var currentLength = listeners.length;
    if (listeners.push(objectRef) > currentLength)
        if(typeof successCB == "function")successCB("MediaPlay API addListener: " +  objectRef.from)
    else
        if(typeof errorCB == "function")errorCB("MediaPlay API failed addListener");
