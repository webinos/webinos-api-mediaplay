...
(function(){
    ...
    var scanDevice = function(devices, that){
        console.log('MediaPlay API: UPnP scan hits timeout finding ' + devices.length + ' device(s).');
        console.log(devices);
        for (var i=0; i<devices.length; i++)
        {
            ...
            if(!that.registeredModules[devices[i].uuid])
            {
                ...
                console.log("MediaPlay API: XBMC implementation has been loaded for device " + devices[i].friendlyName + " at " + devices[i].ip);
            }
        }
        for (var i in that.registeredModules)
        {
            ...
            if(!found && typeof that.registeredModules[i] !== 'function'){
                ...
                console.log("MediaPlay API: unregistering " + i);

                if(typeof that.registeredModules[i].connection != 'undefined'){
    ...
    MediaApiModule.prototype.init = function(register, unregister)
    {
        ...
        if(process.platform === "linux" || process.platform === "darwin"){
            ...
            players.mplayer.on("close", function(exitCode){
                if(exitCode === 0){
                    ...
                    console.log("MediaPlay API: mplayer implementation has been loaded");
                }
                else if(players.omx.exitCode !== 0)
                    console.log("MediaPlay API: supported players not found on this system");
            });
            players.omx = exec("hash omxplayer");
            players.omx.on("close", function(exitCode){
                if(exitCode === 0){
                    register(new omx_module(this.rpcHandler, this.params));
                    console.log("MediaPlay API: omxplayer implementation has been loaded");
                }
            ...
            console.log("MediaPlay API: Android player implementation has been loaded");
        }
        else if(process.platform === "win32"){
            register(new wmplayer_module(this.rpcHandler, this.params));
            console.log("MediaPlay API: Windows Media player implementation has been loaded");
        }
        ...
        console.log("MediaPlay API: UPnP/DLNA player implementation has been loaded");
    };
