...
upnpScan.prototype.scan = function(timeout, parent, callback){
    ...
    setTimeout(function(){
        ...
        var reqClients=[];        
        var httpTimerID = setTimeout(function(){
            console.log("MediaPlay: UPnP http request hits timeout and exit");
            for(var i=0;i<reqClients.length;i++)
                reqClients[i].abort();
        }, 5000);
        
        for(var i=0, j=0; i<responses.length; i++)
        {            
            reqClients[i] = http.get(responses[i].href, function(res) {
                var data = '';
