...
WMplayerModule.prototype.startPlay = function(path, successCB, errorCB) 
{
	...
	//Probably not needed. WMP does not open multiple instances.
        ...
        switch(key.toString())
        {
            ...
            case "q":                
                if(playerInstance.instance.command("stop") !== false){                    
                    playerInstance.playing = null;  ////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!shouldn't this be set here?!
...
{            
    ...
    {   
        var diff = playerInstance.volume-Math.round(volume[0]*volumeRange.step/3);
        if(diff<0)
