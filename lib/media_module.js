...
this.startplay = function(path, successCB, errorCB) {
    ...
    //
    //
    // ATTENTION TO THE FOLLOWING SECURITY FLAW!
    //
    // A passed parameter path[0]==="`echo "hacked">/home/pi/textfile`" will be evaluated on target machine!
    //
    //
    var child = exec('omxplayer -o hdmi "' + path[0] + '" --vol -2400 < ' + DEFAULT_PATH + pipe);
    child.on("close", function () {
        //clean up the fifo unconditionally
    ...
    if (child.pid && child.exitCode==null) {
        if(typeof successCB === 'function')successCB("successCB: OmxPlayer playing with PID " + child.pid);
    } else {
        if(typeof errorCB === 'function')errorCB("omxplayer start unsuccessful");
    }
};
